...
try {
    const options = {
        ...
        from: new RegExp(`process\\.env(?:(?:\\.(${ALLOWED_KEYS.join('|')}))|(?:\\[['"](${ALLOWED_KEYS.join('|')})['"]\\]))`, 'g'),
        to: (match, p1, p2) => {
            const key = p1 || p2;
    ...
    console.log('Environment variables injected:', results.filter(r => r.hasChanged).map(r => r.file));
} catch (error) {
