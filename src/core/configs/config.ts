...
export class AuthConfigManager {
	...
	private loadConfig(): AppConfig {
		...
		return {
			baseURL: envConfig.getSetting('OCTOPUS_API_URL') || 'http://localhost:5000/api',
			authEnabled,
			environment,
			auth: {
				auth0Domain: envConfig.getSetting('AUTH0_DOMAIN') || 'your-domain.auth0.com',
				auth0ClientId: envConfig.getSetting('AUTH0_CLIENT_ID') || 'your-client-id',
				auth0Audience: envConfig.getSetting('AUTH0_AUDIENCE') || 'https://your-api.com',
			...
			network: {
				retry: {
					...
					maxDelay: parseInt(envConfig.getSetting('NETWORK_RETRY_MAX_DELAY') || '10000'),
