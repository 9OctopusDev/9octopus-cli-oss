...
const TokenUsageDisplay: React.FC<TokenUsageDisplayProps> = ({
	...
	if (compact) {
		...
		if (tokenUsage) {
			const {input_tokens, output_tokens, total_tokens, tool_tokens} =
				tokenUsage;

			return (
				<Text color="gray">
					Tokens: {formatTokens(input_tokens)} → {formatTokens(output_tokens)}
					{tool_tokens > 0 && ` (+${formatTokens(tool_tokens)} tools)`} ={' '}
		...
		<Box flexDirection="column" marginTop={1}>
			{tokenUsage && (
				<Box flexDirection="row" gap={2}>
					...
					<Text>
						{formatTokens(tokenUsage.input_tokens)} →{' '}
			...
			{sessionTotal && (
				<Box flexDirection="row" gap={2}>
					...
					<Text>{formatTokens(sessionTotal.total_tokens)} tokens</Text>
				</Box>
			)}
		</Box>
	);
