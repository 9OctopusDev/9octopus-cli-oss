...
const InputComponent: React.FC<InputComponentProps> = ({
    ...
    const [tokenUsage, setTokenUsage] = useState(sessionManager.getCurrentSession().tokenUsage);
    ...
    useInput((input, key) => {
        ...
        if (autocomplete.length > 0) {
            ...
            if (key.upArrow) {
                setSelectedIndex(prev => (prev > 0 ? prev - 1 : autocomplete.length - 1));
                return;
            }
            if (key.downArrow) {
                setSelectedIndex(prev => (prev < autocomplete.length - 1 ? prev + 1 : 0));
                return;
            }
        }
    ...
    const handleInputChange = useCallback((input: string, resetSelection = true) => {
        if (resetSelection) {
            setSelectedIndex(0);
        }
        ...
        if (isSlashCommand(input)) {
            ...
            if (commandName === '') {
                ...
                const exactCommand = commandName ? commandAgregator.getCommand(commandName) : null;

                if (exactCommand && exactCommand.subcommands && (commandParts.length === 1 || (commandParts.length === 2 && subcommandPart === ''))) {
                    // Show subcommands for exact command match when user types "/models" or "/models "
                    setAutocomplete(
                        exactCommand.subcommands.map(subcommand => {
                            const optionsText = subcommand.options.length > 0
                ...
                } else if (exactCommand && exactCommand.subcommands && subcommandPart) {
                    ...
                    const commands = commandName ? commandAgregator.searchCommand(commandName) : [];
            ...
            try {
                ...
                if (!modelInfo.effectiveModel) {
                    addHistoryItem({
                        id: new Date().toLocaleDateString(),
                        role: 'system',
                        content: 'No model available. Please set a default model with `/models default set <provider> <model>` or use `/chat <provider> <model> <message>` format.',
                        timestamp: new Date(),
                    });
                    return;
                }
        ...
        <Box flexDirection="column" width="100%" marginTop={0} paddingTop={0}>
            ...
            <Box marginLeft={2} marginTop={1} flexDirection="row" gap={4}>
                ...
                {modelManager.hasDefaultModel() && (
                    <Text color="green">
                        Model: {modelManager.formatModelDisplay(modelManager.getDefaultModel()!)}
                    </Text>
                )}
                {!modelManager.hasDefaultModel() && (
                    <Text color="yellow">
                        Model: None (use /models set to configure)
                    </Text>
                )}
                <TokenUsageDisplay
                    compact={true}
