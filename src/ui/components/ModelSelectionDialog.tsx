...
const ModelSelectionDialog: React.FC<ModelSelectionDialogProps> = ({
    ...
    useInput((_, key) => {
        if (key.escape) {
            ...
            if (selectedModel) {
                onModelSelected(selectedModel.provider.toLowerCase(), selectedModel.modelName);
            }
        }
    });
    ...
    const loadAvailableModels = async () => {
        try {
            setIsLoading(true);
            const response = await backendApiService.getAllModels();


            if (response.status === 'success' && response.data && response.data.providers) {
                ...
                setError(`Failed to load available models. Response status: ${response.status}, has providers: ${!!response.data?.providers}`);
            }
        } catch (err) {
    ...
    return (
        ...
        >
            ...
            <Box marginBottom={1}>
                <Text color="gray">
                    Choose a model to set as your default. Use ↑↓ arrows to navigate, Enter to select, ESC to cancel.
                </Text>
            </Box>

            <Box flexDirection="column" height={10}>
                {models.slice(Math.max(0, selectedIndex - 4), selectedIndex + 6).map((model, displayIndex) => {
